...
def test_perform_discount_calculation(driver):
    ...
    @pytest.mark.timeout(175)
    def test_perform_2nd_discount_calculation(driver):
        discount_calculator_page = DiscountCalculatorPage(driver)

        with appium_transaction("Perform 2nd Discount Calculation"):
            try:
                try:
                    print("Clearing previous input on the Discount Page")
                    discount_calculator_page.clear_entries()
                ...
                print("Will calculate the amount saved, tax and final price using the calculator")
                second_output = discount_calculator_page.calculate_discount("20", "475", "18")
                print(
                    "Calculated the amount saved, tax and final price using the calculator - will confirm the values are 85.50, 77.90, 467.40")
                second_amount_saved, second_tax, second_price = second_output
                assert float(second_amount_saved) == 5.0, f"Amount saved mismatch: {second_amount_saved}"
                assert float(second_tax) == 14.25, f"Tax mismatch: {second_tax}"
                assert float(second_price) == 109.25, f"Price mismatch: {second_price}"
            except Exception:
                capture_error_snapshot(driver, "Perform2ndDiscountCalculation")
                raise
